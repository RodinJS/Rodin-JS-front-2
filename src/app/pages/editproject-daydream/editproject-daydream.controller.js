class EditProjectDaydreamCtrl {
    constructor (AppConstants, Project, $state, $interval, $stateParams, $q, $scope, User, JWT, EventBus, ProjectStore, Validator, Notification) {
        ...
        ProjectStore.subscribeAndInit($scope, ()=> {
            ...
            if (this.project && this.project.fields && this.project.fields.daydream &&
                (!this.project.daydream || Object.keys(this.project.daydream).length <= 0 ) && !this.project.build.daydream.built) {

                this.project.daydream = {
                    name: this.project.fields.appName,
                    version: this.project.fields.version,
                    package: this.project.fields.daydream.package,
                    keyStore: this.project.fields.daydream.keyStore
                };
                this.projectError = this.project.fields.error;

                if (this.projectError) {
                    this.project.build.daydream.requested = false;
    ...
    getProject () {
        this.showLoader = true;
        this.Project.get(this.projectId, { device: 'daydream' }).then(
            project => {
                this.showLoader = false;
                this.eventBus.emit(this.eventBus.project.SET, project);
                if (this.project.build.daydream.built && this.timer) {
                    this._$interval.cancel(this.timer);
                }
            },
   ...
   publishNbuild(e) {
    ...
    build (event) {
        ...
        let project = {
            ...
            url: 'http://google.com',
            appName: this.project.daydream.name,
            daydream: this.project.daydream,
            version: this.project.daydream.version,
        };
        ...
        $('#configs').ajaxForm({
            dataType: 'json',
            url: this._AppConstants.API + '/project/' + this.project._id + '/build/daydream',
            headers: {
            ...
            success: function (data) {
                ...
                ctrl.getProject();
                ctrl._$scope.$apply();
                ctrl.Notification.success('Daydream build start');
    ...
    download () {
        this.showLoader = true;
        this.Project.download(this.project._id, 'daydream').then(
            data => {
                this.showLoader = false;
...
export default EditProjectDaydreamCtrl;